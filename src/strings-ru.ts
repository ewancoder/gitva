...
export const ru: Strings = {
  ui: {
    ...
    'help.legend.tag': 'аннотированная метка',
  ...
  inspector: {
    ...
    kinds: {
      ...
      tag: {
        title: 'Аннотированная метка',
        what: 'Аннотированная метка — настоящий объект: имя, tagger — кто её поставил, — сообщение и sha того, на что она указывает. Это почти указатель, к которому прилагается история, — поэтому здесь она стоит с указателями, а не с объектами.',
        made: 'git mktag  /  git tag -a <name>',
      },
      ref: {
        title: 'Ссылка',
        what: 'Ссылка — это файл, в котором лежит sha. В этом весь механизм. Ветка — ссылка в refs/heads, которую перезаписывают при каждом коммите; неаннотированная метка — ссылка в refs/tags, которую не перезаписывают. Ничто в ветке не является полноценным объектом.',
