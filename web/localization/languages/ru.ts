...
export const ru: Strings = {
    ui: {
        'recording-id.title':
            'Уникальный ID записи репозитория. Нажмите, чтобы скопировать: `gitva --id ID` подхватит ту же запись откуда угодно',

        'expand-all': 'развернуть всё',
        'expand-all.title':
            'Развернуть каждый коммит и дерево (двойной щелчок по коммиту или дереву разворачивает и сворачивает его)',
        ...
        'links-from-unreachable': 'связи от недостижимых',
        'links-from-unreachable.title': 'Показывать связи от недостижимых объектов к достижимым',
        names: 'имена',
        ...
        'help.legend.submodule': 'подмодуль',
        ...
        'help.keys.doubleCommit.does':
            'развернуть или свернуть — показать, на что ссылается этот коммит',
        ...
        'help.keys.columnEdge': 'перетащить границу столбца',
        'help.keys.columnEdge.does': 'расширить этот столбец («сбросить вид» вернёт его назад)',
        ...
        'settings.lede': 'Личные настройки просмотра.',
    ...
    canvas: {
        columns: {
            pointersAndTags: 'указатели и теги',
            commits: 'коммиты',
            treesAndBlobs: 'деревья и блобы',
            index: 'индекс',
        },
        tagPrefix: 'тег: ',
    ...
    notes: {
        ...
        unreachableHidden: 'Недостижимые объекты скрыты — они по-прежнему в базе данных объектов.',
        bodiesOnSelection: 'Содержимое объектов загружается, когда вы что-нибудь выбираете.',
    },
    ...
    inspector: {
        ...
        truncated: (size: number) =>
            `… первые 64 КиБ из ${size} ${plural(size, 'байта', 'байтов', 'байтов')}.`,
        size: {
            bytes: (n: number) => `${n} Б`,
            kib: (n: string) => `${n} КиБ`,
        },
        ...
        kinds: {
            ...
            indexSubmodule: {
                title: 'Запись подмодуля',
                what: 'Подмодуль — это одна запись индекса с режимом 160000, хранящая sha коммита, и этот коммит живёт в другом репозитории, а не в этом. В этом весь механизм: главный репозиторий записывает, какой коммит подмодуля ему нужен, и больше ничего о нём. Читать здесь нечего — этой базе данных такой объект не принадлежит.',
