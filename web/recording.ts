...
export type Arrival = { kind: 'none' } | { kind: 'shown'; prev: Step | null; first: boolean };
...
export class Recording {
        ...
        if (first) {
            ...
            this.view = {
                ...
                expanded: this.answered(
                    this.learning ? openAll : settings.expandNewCommits ? [...this.born] : [],
                ),
            };
    ...
    toggleTree(oid: Oid) {
        ...
        this.view = {
            ...this.view,
            collapsed: off.includes(oid) ? off.filter((o) => o !== oid) : [...off, oid],
        };
    }
...
export function isDouble(prev: Click | null, now: Click): boolean {
    return (
        !!prev &&
        now.at - prev.at < 500 &&
        Math.abs(now.x - prev.x) < 12 &&
        Math.abs(now.y - prev.y) < 12
    );
}
