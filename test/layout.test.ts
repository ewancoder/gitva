...
describe('a commit\'s objects', () => {
  ...
  it('keeps a blob a collapsed tree shared with someone still open', () => {
    ...
    assert.equal(g.links.some((e) => e.from === 'lib'), false);
  });
  ...
  it('writes both names on one link when two files in a tree are the same blob', () => {
    ...
    assert.deepEqual(g.links.map((e) => e.label), ['a.txt, b.txt']);
  });
  ...
  it('marks an executable and a symlink on the link', () => {
    ...
    assert.deepEqual(g.links.map((e) => e.label), ['run.sh +x', 'link ->']);
  });
});
...
describe('the scene', () => {
  ...
  it('collapsing a tree removes what it holds from the scene, and says how much', () => {
    ...
    assert.equal(after.shapes.find((n) => n.id === oid('ta'))?.collapsed, true, 'and says so, for the painter');
    assert.equal(after.shapes.some((n) => n.id === oid('bl')), false, 'hidden means absent');
    assert.equal(after.links.some((e) => e.to === oid('bl')), false);
  });
  ...
  it('accordions a row open, pushing everything below down by a predictable amount', () => {
    ...
    assert.equal(open.rows[1].y - collapsed.rows[1].y, grew, 'and the row below shifted by exactly that');
  ...
  it('still right-aligns the chips against the narrower gutter', () => {
    ...
    for (const n of scene.shapes.filter((c) => c.kind === 'ref' || c.kind === 'head')) {
      assert.ok(n.x >= column.x && n.x + n.w <= column.x + column.w, `${n.id} is inside the gutter`);
    }
  ...
  it('draws no link to a parent outside the window', () => {
    ...
    assert.ok(!scene.links.some((e) => e.to === oid('gone')), 'never a link to a shape that is not there');
  ...
  it('hides the index outright when it is switched off', () => {
    ...
    assert.ok(!off.shapes.some((n) => n.kind === 'index'), 'taken out of the drawing, not merely invisible');
  ...
  it('collapses an unreachable tree too — a ghost is a tree with entries like any other', () => {
    ...
    assert.equal(scene.links.some((e) => e.to === oid('lb')), false, 'and no link into it from the collapsed tree');
  });
  ...
  it('hides unreachable objects outright when they are switched off, and keeps staged blobs', () => {
    ...
    assert.ok(!off.shapes.some((n) => n.id === oid('lost')), 'out of the scene, not merely invisible');
    assert.ok(off.shapes.some((n) => n.id === oid('stag')), 'the index still holds this one');
  });
  ...
  it('keeps the links between objects that were unreachable together', () => {
    ...
    assert.ok(!scene.shapes.some((n) => n.id === kept), 'the live blob is not dragged down here');
  ...
  it('puts an unreachable tag in the pointer gutter, beside what it still names', () => {
    ...
    assert.ok(scene.links.some((e) => e.from === tagged && e.to === lost && e.kind === 'pointer'));
  ...
  it('keeps a staged blob beside the unreachable tree that names it — git write-tree', () => {
    ...
    assert.ok(scene.links.some((e) => e.to === blob && e.kind === 'stage'), 'the chip still holds it');
  });
  ...
  it('hoists a staged blob whose only tree is an unreachable object that is switched off', () => {
    ...
    assert.ok(shape(blob).y < shape(oid('a')).y, 'so the blob goes back up beside the newest commit');
  });
  ...
  it('draws an unreachable tree\u2019s links into live territory only when asked', () => {
    ...
    assert.equal(on.shapes.filter((n) => n.id === live).length, 1, 'the blob is not drawn twice');
    assert.deepEqual(on.shapes, off.shapes, 'a link is added, nothing is moved');
    assert.ok(on.links.some((e) => e.from === lost && e.to === own), 'its own blob still fans out');
    // Collapse the commit away and the blob is off the screen; a link to a shape
    // nobody can see is not a link.
    assert.equal(fromUnreachable(layout(s, { ...DEFAULT_VIEW, showLinksFromUnreachable: true })).length, 0);
  });
  ...
  it('draws a discarded commit\u2019s link to the live parent it was cut from', () => {
    ...
    assert.equal(parent(DEFAULT_VIEW).length, 0, 'off by default, like any link from unreachable');
  ...
  it('says nothing about the entries of a collapsed unreachable object, links from unreachable or not', () => {
    ...
    const scene = layout(s, { ...DEFAULT_VIEW, expanded: [oid('a')], showLinksFromUnreachable: true, collapsed: [lost] });
  ...
  it('puts lost commits newest first, and settles a tie by sha', () => {
    ...
    assert.ok(y(recent) < y(tied), 'two at the same moment go in sha order, so the canvas is stable');
  });
  ...
  it('lets a pin override a position without disturbing anything else', () => {
    ...
    assert.deepEqual(other, free.shapes.find((n) => n.id === oid('a')));
  });
