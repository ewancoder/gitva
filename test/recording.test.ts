...
describe('the recording', () => {
    ...
    it('starts over when the recording it is handed is numbered from one again', () => {
        ...
        assert.equal(
            over.kind,
            'shown',
            'the first step of the new recording was taken for a re-send',
        );
...
describe('what is collapsed', () => {
    ...
    it('leaves the links from unreachable as you last left them, demonstration or not', () => {
        ...
        assert.equal(
            t.view.showLinksFromUnreachable,
            false,
            'the flag overruled an answer already given',
        );
...
describe('what the toolbars say', () => {
    it('counts what is on screen, and what the repository holds', () => {
        ...
        assert.equal(
            t.tally(12),
            '12 on screen · 8 commits · 1c 1t 1b 1g · 1 unreachable · 0 index',
        );
    });
...
describe('pins', () => {
    ...
    it('brings pins back after a reload, holding from the first step there is', () => {
        ...
        after.restore(
            JSON.parse(JSON.stringify(before.all)) as { id: string; x: number; y: number }[],
        );
...
describe('the recording from before this browser arrived', () => {
    it('replays it whole, standing the session’s commits back up and asking nothing', () => {
        ...
        assert.deepEqual(
            t.view.expanded,
            [oid('b'), oid('c')],
            'a reload collapsed the session’s commits',
        );
        assert.ok(
            t.shown?.trees[oid('tb')],
            'their trees came with the steps, so nothing has to be asked for',
        );
        ...
        assert.deepEqual(
            t.view.expanded,
            [oid('b'), oid('c'), oid('d')],
            'the commit git just made stayed collapsed',
        );
    });
    ...
    it('hands back what was expanded by hand, whatever the step says', () => {
        ...
        assert.ok(
            ...
            'a reload collapsed what was expanded by hand',
        );
    ...
    it('keeps a commit collapsed by hand where everything else is open', () => {
        ...
        assert.ok(
            !reloaded.view.expanded.includes(oid('a')),
            '--learning re-opened a commit somebody had collapsed',
        );
        assert.ok(
            reloaded.view.expanded.includes(oid('b')),
            'the rest of the recording lost its expanded commits',
        );
    });
    ...
    it('leaves collapsed whatever was collapsed by hand, however much later', () => {
        ...
        assert.deepEqual(
            t.view.expanded,
            [oid('c')],
            'the replay re-opened a commit somebody had collapsed',
        );
    });
    ...
    it('does not call a commit new because it dropped out of the window and came back', () => {
        ...
        assert.deepEqual(
            t.view.expanded,
            [oid('b')],
            'a commit older than this browser was opened',
        );
    });
    ...
    it('leaves the repository as it stood before the session, when that is the setting', () => {
        ...
        assert.deepEqual(
            t.view.expanded,
            [],
            'replay opened commits the setting said to leave collapsed',
        );
    });
});
